            ...
            {users.filter(user => user._id !== currentUser._id).map(user => (
                <div
                    ...
                    className={`flex items-center p-3 rounded-xl cursor-pointer transition-all duration-200
                        ${selectedUser && selectedUser._id === user._id ? 'bg-primary-light text-white shadow-md' : 'bg-slate-50 hover:bg-slate-100'}
                    `}
                >
                    ...
                    <div className="ml-3">
                        <p className={`font-semibold ${selectedUser && selectedUser._id === user._id ? 'text-white' : 'text-slate-800'}`}>{user.username}</p>
                        <p className={`text-sm ${selectedUser && selectedUser._id === user._id ? 'text-white' : 'text-slate-500'}`}>
